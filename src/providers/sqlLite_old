...
export class SqlLiteData {
    ...
    constructor(public userData: UserData) {
        ...
        this.database.openDatabase({
            ...
            .then(() => this.refresh())
            .catch(err => console.error("ERROR openDatabase: ", err))
    }
    /**
     * [update description]
    ...
    updateUpload(data) {
        ...
        this.database.executeSql(request, [])
    ...
    refresh(): Promise<any> {
        return new Promise((resolve, reject) => {
            this.database.executeSql("SELECT * FROM people", [])
                .then(data => {
                    this.people = []
    ...
    delete(uri) {
        return new Promise((resolve, reject) => {
            this.database.executeSql("DELETE FROM people WHERE uri = '" + uri + "'", [])
                .then(data => {
                    this.refresh()
                    resolve(data)
                })
            ...
            this.getProfile()
                .then(result => {
                    ...
                    this.database.executeSql(requestSql, [
                        ...
                        .catch(err => reject("ERROR requestSql: "))
                })
                .catch(err => reject("ERROR getProfile: ")
                )
        });
    }
    ...
    recreate() {
        ...
        this.database.executeSql("DROP TABLE people", [])
