...
export class FileService {
    ...
    checkFileJsonMeta(uriExtractFile: string) {
        return new Promise((resolve, reject) => {
            this.file.checkFile(cordova.file.dataDirectory, uriExtractFile)
                .then(result => {
                    this.file.removeFile(cordova.file.dataDirectory, uriExtractFile)
    ...
    checkFileTxt() {
        return new Promise((resolve, reject) => {
            ...
            this.file.checkFile(cordova.file.dataDirectory, "fin.txt")
                .then(result => resolve())
                .catch(err => {
                    this.file.createFile(cordova.file.dataDirectory, "fin.txt", true)
            ...
            this.checkFileJsonMeta(uriExtractFileWithoutExt + ".json")
                .then(value => {
                    this.file.writeFile(
                        ...
                        JSON.stringify(item),
                        true)
                        .then(() => resolve(cordova.file.dataDirectory + uriExtractFileWithoutExt + ".json"))
                        .catch(err => reject(err))
                })
                .catch(err => reject(err))
        })
